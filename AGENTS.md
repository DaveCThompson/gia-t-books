...
1.  **Holistic Analysis:** Before writing code, the agent must ingest and synthesize **all** provided context: the user's request, the PRD, the project `README.md`, `STYLING.md`, and all relevant existing code files. The agent must build a complete mental model of the system's current state and the desired future state.
...
2.  **Impact Analysis & Dependency Mapping:**
    ...
    *   Map the dependencies. For example: "Updating the `Page` component will require passing new props from the `BookReader` feature." This prevents leaving dependent files in a broken state.

3.  **Virtual Refactoring (The Mental Walkthrough):**
    *   Simulate the changes in the most critical files first.
    *   **Example Simulation (Styling):** *"I need to style the `BookCover` component. I will use the co-located `BookCover.module.css` file and import the `styles` object. I will apply `className={styles.coverImage}` to the `img` tag to ensure the styles are locally scoped and will not conflict with other image styles in the application."*
    ...
    *   **Example Simulation (Type Safety):** *"I am calling `useRef<number>()`. The build fails because the type signature requires an initial value. The idiomatic solution is to type the ref as `useRef<number | null>(null)` to correctly model its initially empty state."*
